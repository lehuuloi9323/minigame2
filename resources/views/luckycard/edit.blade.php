...
<div id="content" class="container-fluid">
    <div class="card">
        <div class="card-header font-weight-bold">
            Sửa giá trị phần thưởng
        </div>
        <div class="card-body">
            @if (!empty(session('status')))
            <div class="alert alert-success">{{ Session('status') }}</div>
            @endif
            <form method="POST" action="{{ route('lucky_card.update', $luckycard->id) }}">
                @csrf
                <div class="form-group">
                    <label for="name">Tên phần thưởng</label>
                    <input class="form-control" type="text" name="name" id="name" value="{{ $luckycard->name }}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" type="number" name="percent" id="percent" value="{{ $luckycard->percent }}">
                    @error('percent')
                    <small class="text-danger d-block">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input id="thumbnail" name="url" class="form-control" type="text" value="{{ $luckycard->url }}">
                      </div>
